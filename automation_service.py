...
def calcular_data_alvo(data_base, dias_apos, eh_aniversario=False):
    ...
    # Tratamento robusto de erro de data (String ou Date)
...
def processar_regras():
    ...
    if not conn: return
    hoje = datetime.now().date()
    # Define limite: Mensagens de até 3 dias atrás (para cobrir fim de semana), mas não mais que isso.
        ...
        with conn.cursor(pymysql.cursors.DictCursor) as cursor:
            ...
            for r in regras:
                ...
                for p in lista:
                    ...
                    # CORREÇÃO: Envia apenas se for HOJE ou no máximo 3 dias atrás (ignorando coisas velhas)
...
def main_loop():
    ...
    while True:
        now = datetime.now()
        # Verifica a cada 15 min em horário comercial
