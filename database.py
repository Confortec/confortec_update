...
def check_and_fix_table_schema(cursor, table_name, expected_columns_sql):
    try:
        ...
        # VERIFICAÇÃO TABELA PRODUCT_CACHE (Adicionado para suportar COD)
...
def initialize_database():
    ...
    try:
        with conn.cursor() as cursor:
            # Tabelas existentes do seu sistema
            cursor.execute("""
            ...
            # Tabela de Cache (Atualizada com COD)
            cursor.execute("""
            ...
            """)
            # Executa verificação para adicionar coluna COD em bancos existentes
...
def get_all_products_from_cache():
    """Retorna TODOS os produtos salvos no banco para carregamento rápido."""
    conn = get_db_connection()
    if not conn: return []
    try:
        with conn.cursor() as cursor:
...
def save_product_to_cache(data):
    ...
    try:
        ...
        cod = data.get('cod', '') # Pega o código do scraper

        with conn.cursor() as cursor:
            # Inclui COD no Insert e no Update
            cursor.execute("""
