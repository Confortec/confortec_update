...
def limpar_texto(texto):
    """Normaliza o texto removendo sujeira e espaços extras."""
    if not texto: return ""
...
def converter_preco_float(valor_str):
    """
    Converte '2.500,00' ou 'R$ 2.500,00' para float 2500.00.
...
def validar_produto(soup, url):
    """
    Valida se o produto é útil (Tem Título e Preço).
    """
    ...
    for tentativa in range(3):
        try:
            ...
            if price_div:
                ...
                if vals: 
                    # Pega o último valor encontrado (preço atual)
                    str_valor = vals[-1]
                    details['preco_vista'] = str_valor
                    valor_vista_float = converter_preco_float(str_valor)

            # --- DEFINIÇÃO DA ÁREA DE BUSCA ---
            # Para garantir que vamos achar o COD, pegamos o texto do container principal do produto
            # Se não achar o container product, pega o body inteiro (segurança)
            ...
            # --- 3. LOG DE AUDITORIA (VERIFICAÇÃO) ---
            print("-" * 80)
...
def buscar_produtos_site():
    ...
    for cat_url in urls_categorias:
        ...
        while page <= 20:
            ...
            try:
                # print(f"... pág {page}") # Reduzido para limpar console
